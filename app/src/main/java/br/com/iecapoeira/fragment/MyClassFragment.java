    ...
    public void init() {
        ...
        pager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                ((MyClassListFragment_)adapter.getItem(pager.getCurrentItem())).update();
...
//        private int[] iconsSelected = {R.drawable.ic_eventlist_tab_globo_select, R.drawable.ic_eventlist_tab_mapa_selected, R.drawable.ic_eventlist_tab_agend_selected};
        ...
        public Fragment getItem(int position) {
            if (fragments[position] == null) {
                switch (position) {
                    case 0:
                        MyClassListFragment.LIST =0;
                        fragments[position] = MyClassListFragment_.builder().listType(MyClassListFragment.LIST_BY_ANGOLA).build();
                        break;
                    case 1:
                        MyClassListFragment.LIST =1;
                        fragments[position] = MyClassListFragment_.builder().listType(MyClassListFragment.LIST_BY_REGIONAL).build();
                        break;
                    default:
                        return null;
                }
        ...
        public void update() {
            for (Fragment fragment : fragments) {
                try {
                    ((MyClassListFragment) fragment).update();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
